    ...
    <main className="admin-main">
      ...
      {showForm && <section className="admin-editor">
        ...
        <form onSubmit={saveProduct}>
          ...
          <label className="admin-upload field-wide"><ImagePlus /><span><b>{imageFile ? imageFile.name : form.image_url ? 'Trocar fotografia' : 'Adicionar fotografia'}</b><small>JPG, PNG, GIF, WebP ou AVIF — máximo de 5 MB</small></span><input type="file" accept="image/jpeg,image/png,image/gif,image/webp,image/avif" onChange={event => setImageFile(event.target.files?.[0] ?? null)} /></label>
